...
const OrderItem =(props) => {
      ...
      const handleOrder = async (data) => {   
            ...
            history.push('/historyorderdetailin')
      }
      return (
...
function HistoryIn() {
      ...
      const [orderIn, setOrderIn] = useState(null)
            ...
            if(!isAmounted) { 
                  Promise.all([getUSER(), getTOKEN()]).then((res) => {
                        ...
                        if(userData !== null && tokenData !==null){
                              Promise.all([API.historyin(userData.id, tokenData)]) 
                              .then((result) => { 
                                    // console.log('order', result);
                                    setOrderIn(result[0].data)
                                    setLoading(false) 
                              }).catch((e) => {
            ...
            <Fragment>
                  <Header/>
                        <div className="container">
                            ...
                            {orderIn && orderIn.map((item, index) => {
                                    return (
                                    <OrderItem 
                                        key ={index}
